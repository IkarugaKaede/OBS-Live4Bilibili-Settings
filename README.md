...
|:---|:---|:---|
| 录像机 | 视频捕获设备 | 主播摄像头画面 |
| 窗口采集 | 窗口捕获 | 特定窗口内容（如聊天窗口、浏览器） |
| MC | 游戏捕获 | Minecraft 专用捕获 |
| 显示器采集 | 显示器捕获 | 全屏备份捕获 |
| Loading... | 图像/媒体源 | 转场/加载画面覆盖层 |
| 游戏画面 | 游戏捕获 | 通用游戏捕获 |
| 游戏音频 | 音频输出捕获 | 游戏内音效捕获 |
| 音频 | 音频输入/输出 | 通用系统音频 |
| 麦克风 | 音频输入捕获 | 主播语音输入 |
| OOPZ语音 | 应用程序音频采集 | 开黑语音软件音频 |
| 网易云音乐 | 应用程序音频采集 | 背景音乐 |
...
- **冗余捕获策略**：同时配置 `MC`、`游戏画面`、`显示器采集` 三种视频捕获源，通过可见性切换实现不同内容快速转场
- **音频分离控制**：语音（麦克风 + OOPZ语音）、音乐（网易云音乐）、游戏音效各自独立通道，便于动态混音
- **转场覆盖层**：`Loading...` 来源用于直播等待/转场场景
